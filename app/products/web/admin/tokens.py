...
def _quota_brief(q: dict) -> dict:
    """Extract quota modes with only remaining/total from stored quota dict."""
    out = {}
...
async def edit_token(
    ...
    await repo.patch_accounts([AccountPatch(
        ...
        quota_heavy=qs.heavy.to_dict() if qs.heavy is not None else None,
        quota_grok_4_3=qs.grok_4_3.to_dict() if qs.grok_4_3 is not None else None,
        quota_console=qs.console.to_dict() if qs.console is not None else None,
